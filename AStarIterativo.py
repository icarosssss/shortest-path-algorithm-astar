...
def ibf_a_star(graph, start, goal, max_iterations):
    frontier = []
    ...
    while frontier:
        ...
        for next_node in graph.neighbors(current):
            ...
            if next_node not in cost_so_far or new_cost < cost_so_far[next_node]:
                ...
                priority = ox.distance.euclidean_dist_vec(graph.nodes[next_node]['y'], graph.nodes[next_node]['x'], graph.nodes[goal]['y'], graph.nodes[goal]['x'])
...
came_from, cost_so_far = ibf_a_star(graph, origin_node, destination_node, max_iterations)
