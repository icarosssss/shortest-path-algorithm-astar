...
def ida_star(graph, start, goal):
    ...
    cost_so_far[start] = 0
    threshold = ox.distance.euclidean_dist_vec(graph.nodes[start]['y'], graph.nodes[start]['x'], graph.nodes[goal]['y'], graph.nodes[goal]['x'])
    ...
    while frontier:
        ...
        if cost_so_far[current] + ox.distance.euclidean_dist_vec(graph.nodes[current]['y'], graph.nodes[current]['x'], graph.nodes[goal]['y'], graph.nodes[goal]['x']) > threshold:
            new_threshold = float('inf')
            for node in frontier:
                node_cost = cost_so_far.get(node[1], float('inf')) + ox.distance.euclidean_dist_vec(graph.nodes[node[1]]['y'], graph.nodes[node[1]]['x'], graph.nodes[goal]['y'], graph.nodes[goal]['x'])
                if node_cost < new_threshold:
                    new_threshold = node_cost
...
came_from, cost_so_far = ida_star(graph, origin_node, destination_node)
